...
# Total:                                                  95
#
# WS=23G, CXL=18G, DRAM=3G, DAX=dax0.6
# =============================================================================
...
RESULT_DIR="$SCRIPT_DIR/run_${TIMESTAMP}_exp7_comparison"
mkdir -p "$RESULT_DIR"
...
# two_level: A/B/C/F/TPC-C → RC:BP = 6:1
TWO_LEVEL_BP_ABCF=0.43
TWO_LEVEL_RC_ABCF=2.57
# two_level: E only → RC:BP = 1:6 (D uses A/B/C/F split)
TWO_LEVEL_BP_DE=2.57
TWO_LEVEL_RC_DE=0.43
# baselines: all DRAM to BP
BASELINE_BP=3.0
...
run_tpcc() {
   ...
   # TPC-C two_level uses A/B/C/F split (RC-friendly)
...
# -----------------------------------------------------------------------------
main() {
   log_phase "Exp7 Comparison (WS=${WS_GIB}G, CXL=${CXL_GIB}G, DRAM=${DRAM_TOTAL}G)"
   ...
   log_info "thetas           = ${THETAS[*]}"
   log_info "ycsb workloads   = ${YCSB_WORKLOADS[*]}"
   log_info "two_level ABCF   = BP=${TWO_LEVEL_BP_ABCF} RC=${TWO_LEVEL_RC_ABCF}"
   log_info "two_level E      = BP=${TWO_LEVEL_BP_DE} RC=${TWO_LEVEL_RC_DE}"
   log_info "baseline         = BP=${BASELINE_BP}"
   ...
   log_phase "EXP7 COMPARISON COMPLETE"
