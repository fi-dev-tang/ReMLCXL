...
void printer(string name, LeanStoreAdapter<T>& adapter, LeanStore& db)
{
   ...
   cout << "nodes:" << pages << " space:" << pages / (float)(db.getBufferManager().inUsedPageIDCount() / 100) << "% height:" << adapter.btree->getHeight()
        << endl;
}
...
{
   auto& crm = db.getCRManager();
   double gib = (db.getBufferManager().inUsedPageIDCount() * EFFECTIVE_PAGE_SIZE / 1024.0 / 1024.0 / 1024.0);
