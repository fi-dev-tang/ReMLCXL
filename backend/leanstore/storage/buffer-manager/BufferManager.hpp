...
class BufferManager
{
  private:
   ...
   void promoteRecordCacheEntry(u64 page_id, u16 slot_id, BufferFrame* cxl_bf = nullptr, bool is_urgent = false);
  ...
  public:
   ...
   inline BufferFrame& tryFastResolveSwip(Guard& swip_guard, Swip<BufferFrame>& swip_value)
   {
      if (swip_value.isHOT()) {
         ...
         swip_guard.recheck();
         // [CXL-tiering safety] Under heavy cross-tier frame recycling an inner node can be
         // reset/reused mid optimistic-traversal, so this HOT swip slot may hold a stale/torn
         // value that decodes to a wild pointer. Validate bf lies inside a real frame region
         // BEFORE the caller dereferences bf->header.latch; otherwise abort optimistically.
   ...
   struct DiagCounters {
      ...
      // Urgent fast-promote funnel diagnostics
      // (urgent_seen_in_check / urgent_decision_generated / urgent_seen_but_already_promoted
      //  are in DramHotPageCandidates since CheckAndPromote cannot see BMC)
      std::atomic<u64> urgent_not_marked_candidate_missing{0};
      std::atomic<u64> urgent_marked_total{0};

      // Enqueue diagnostics (split by source path)
      std::atomic<u64> urgent_enqueued_to_promote{0};
      std::atomic<u64> normal_enqueued_to_promote{0};

      // promoteRecordCacheEntry() pre-enqueue fail reasons (split urgent/normal)
      std::atomic<u64> urgent_fail_cxl_bf_stale{0};
      std::atomic<u64> urgent_fail_slot_boundary{0};
      std::atomic<u64> urgent_fail_large_record{0};
      std::atomic<u64> normal_fail_cxl_bf_stale{0};
      std::atomic<u64> normal_fail_slot_boundary{0};
      std::atomic<u64> normal_fail_large_record{0};
   } diag;
};                                                    // namespace storage
// -------------------------------------------------------------------------------------
class BMC
