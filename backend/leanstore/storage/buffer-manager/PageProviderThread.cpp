...
#define repickIf(cond)                       \
   if (cond) {                               \
      ...
      //================================ Pull Request ==================================================
      // -------------------------[Added].--------------------------------------------------------------
      // The originial version has a bug here!
      // In pageProviderThread, thread should be responsible for partitions start from [p_begin, p_end)
      // However, calling randomPartition() destory the logic!
      // Error line: auto& current_partition = randomPartition();
      // getRand<u64> has already assert that we can start from >= p_begin, and end at < p_end.
      ...
      for (volatile const auto& cooled_bf : evict_candidate_bfs) {
         jumpmuTry()
         {
            ...
            if (cooled_bf->header.state != BufferFrame::STATE::COOL ||
                cooled_bf->header.is_being_written_back) {  //  || getPageID_AllocatorPartitionID(bf.header.pid) != p_i
               jumpmu_continue;
            }
            const PID cooled_bf_pid = cooled_bf->header.pid;
            const u64 p_i = getPageID_AllocatorPartitionID(cooled_bf_pid);
            // Prevent evicting a page that already has an IO Frame with (possibly) threads working on it.
            {
               Partition& partition = getPageID_AllocatorPartition(p_i);
               ...
               if (!async_write_buffer.full()) {
                  {
                     ...
                     if (FLAGS_out_of_place) {
                        ...
                        paranoid(getPageID_AllocatorPartitionID(cooled_bf->header.pid) == p_i);
                        paranoid(getPageID_AllocatorPartitionID(wb_pid) == p_i);
                     }
