...
struct DTRegistry {
   struct DTMeta {
      ...
      // -------------------------------------------------------------------------------------
      // [Added].
      // In-order-to enable SSD load to CXL's logic:
      // We need to Copy page (pread) from SSD to cxl_free_list's CXL BufferFrame
      // Then occuring to the already persisent page.dt_id information,
      // page.dt_id may correspond to BTreeVI or BTreeLL, 
      // then we call the corresponding instance's registered callback function, to test the is_leaf 
      // [Caution]: 
      // Some data structure can ignore the should_directly_promote_to_dram function pointer, 
      // They can just stay in CXL tier
      // DataStructure's registered callback function.
      // =========================================================================================
      // Relation:
      // Register Type callback
      //     ---> Register Instance callback
      // Whether instance has corresponding callback function?
      //    Check the instance's Type, if exist, pass the Type callback with instance pointer
      // ==========================================================================================
      std::function<bool(void* dt_object, const BufferFrame& bf)> should_directly_promote_to_dram = nullptr;
   };
   // -------------------------------------------------------------------------------------
   ...
   // [Added]. 
   // Instance's function,
   // Currently only for SSD load Index pages to CXL
   // True: if the current page is Index page
   // False: if the current page is Leaf page
   bool shouldDirectlyPromoteToDRAM(DTID data_instance_id, const BufferFrame& bf);
   bool hasInstance(DTID dtid) const;
};
