                                         ...
                                         UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   ...
   jumpmuTry()
   {
      ...
      // Admission control runs pre-update on the page id; it is independent
      // of the RecordCache update path.
      if(FLAGS_cxl_tiering_enabled && BMC::global_bf -> global_record_cache != nullptr){
      ...
      // Write-Through sync: now that the CXL Page holds the new value, push it
      // into the resident RecordCacheEntry under SeqLock.
      if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
         std::span<const u8> key_span(o_key, o_key_length);
         std::span<const u8> new_value_span(current_value.data(), current_value.length());
         BMC::global_bf->global_record_cache->tryWriteThroughSync(
             ...
             static_cast<u16>(cr::Worker::my().workerID()), /*tx_ts=*/0);
      }
