...
OP_RESULT BTreeLL::lookup(u8* key, u16 key_length, function<void(const u8*, u16)> payload_callback)
{
   ...
   if(FLAGS_cxl_tiering_enabled && BMC::global_bf -> global_record_cache != nullptr){
      ...
      std::span<const u8> key_span(key, key_length);
            ...
            //=================================================================================================================
            {
               ...
               if(FLAGS_cxl_tiering_enabled && BMC::global_bf -> global_admission_control != nullptr){
                  BMC::global_bf->getGlobalAdmissionControl().OnRecordAccess(
                     leaf.bf -> header.pid, pos, is_in_dram, leaf -> count, is_in_dram ? nullptr : leaf.bf
                  );
               }
                           ...
                           function<void()>)
{
   ...
   jumpmuTry()
   {
      ...
      PID last_scan_pid = std::numeric_limits<PID>::max();
      OP_RESULT ret = iterator.seek(key);
      while (ret == OP_RESULT::OK) {
         const PID cur_pid = iterator.leaf.bf->header.pid;
         if (cur_pid != last_scan_pid && FLAGS_cxl_tiering_enabled &&
             BMC::global_bf->global_admission_control != nullptr) {
            last_scan_pid = cur_pid;
            const bool is_in_dram = BMC::global_bf->isInDRAM(iterator.leaf.bf);
            BMC::global_bf->getGlobalAdmissionControl().OnPageAccessForScan(
                cur_pid, is_in_dram, iterator.leaf->count, is_in_dram ? nullptr : iterator.leaf.bf);
         }
                                         ...
                                         UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   ...
   jumpmuTry()
   {
      ...
      u64 rc_page_id = 0;
      u16 rc_slot_id = 0;
      //[Added]: key exist, Invalidate RecordCache's old value
      if(FLAGS_cxl_tiering_enabled && BMC::global_bf -> global_record_cache != nullptr){
         u64 worker_id = cr::Worker::my().worker_id;
      ...
      // Trigger urgent re-promotion only after new version is materialized.
...
struct DTRegistry::DTMeta BTreeLL::getMeta()
{
   DTRegistry::DTMeta btree_meta = {.iterate_children = iterateChildrenSwips,
                                    ...
                                    // If it is BTreeLL, then check if it is InnerNode, innerNode should promote to DRAM.
