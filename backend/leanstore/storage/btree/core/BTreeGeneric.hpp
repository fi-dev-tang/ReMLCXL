  ...
  public:
   ...
   static struct ParentSwipHandler findParent(BTreeGeneric& btree, BufferFrame& to_find)
   {
      ...
      auto search_condition = [&](HybridPageGuard<BTreeNode>& guard) {
         ...
         // [FIX] Check EVICTED before dereferencing — concurrent eviction (e.g.,
         // by cxlPageProviderThread) may have changed the Swip to (pid | evicted_bit),
         // making asBufferFrameMasked() dereference a raw PID as a pointer (segfault).
         // Recheck under the parent guard: a single isEVICTED() test is not enough
         // (TOCTOU between the test and asBufferFrameMasked()).
         guard.recheck();
         if (c_swip->isEVICTED()) {
            return false;
         }
         BufferFrame& child_bf = c_swip->asBufferFrameMasked();
         if (c_swip->isEVICTED()) {
            jumpmu::jump();
         }
         return (&child_bf != &to_find);
      };
      ...
      p_guard.unlock();
      // [FIX] Same as above — recheck EVICTED immediately before dereferencing
      if (c_swip->isEVICTED()) {
         jumpmu::jump();
      }
      BufferFrame& candidate_bf = c_swip->asBufferFrameMasked();
      if (c_swip->isEVICTED()) {
         jumpmu::jump();
      }
      const bool found = &candidate_bf == &to_find;
      c_guard.recheck();
      if (!found) {
         jumpmu::jump();
      }
