...
{
   //=======================================================================================================
   // [Added]. Try Lookup Interception in DRAM RecordCache (once, before optimistic/pessimistic fallback)
   //=======================================================================================================
   if(FLAGS_cxl_tiering_enabled && BMC::global_bf -> global_record_cache != nullptr){
      u64 worker_id = cr::Worker::my().worker_id;
      std::span<const u8> key_span(o_key, o_key_length);

      if(BMC::global_bf -> global_record_cache -> tryLookupInRecordCache(dt_id, key_span, payload_callback, worker_id)){
         BMC::global_bf->diag.record_cache_hit.fetch_add(1, std::memory_order_relaxed);
         return OP_RESULT::OK;
      }
      BMC::global_bf->diag.record_cache_miss.fetch_add(1, std::memory_order_relaxed);
   }
   //=========================================================================================================
...
OP_RESULT BTreeVI::lookupPessimistic(u8* key_buffer, const u16 key_length, function<void(const u8*, u16)> payload_callback)
{
   ...
   jumpmuTry()
   {
      ...
      // -------------------------------------------------------------------------------------
      //=================================================================================================================
      // [Added]. Record access to CMS only when the tuple is successfully visible
      //=================================================================================================================
      if(ret == OP_RESULT::OK){
         bool is_in_dram = BMC::global_bf -> isInDRAM(iterator.leaf.bf);
         bool is_in_cxl  = !is_in_dram && FLAGS_cxl_tiering_enabled && BMC::global_bf -> isInCXL(iterator.leaf.bf);

         // OnRecordAccess is only meaningful when the admission control is active (CXL path).
         if(FLAGS_cxl_tiering_enabled && BMC::global_bf -> global_admission_control != nullptr){
            BMC::global_bf->getGlobalAdmissionControl().OnRecordAccess(
               iterator.leaf.bf -> header.pid,
               iterator.cur,
               is_in_dram,
               iterator.leaf -> count,
               is_in_dram ? nullptr : iterator.leaf.bf
            );
         }

         // dram_hit / cxl_hit are mutually exclusive per lookup.
         // ssd_miss is counted separately in BufferManager::resolveSwip at the
         // point where readPageSync is called — after that the frame is already
         // in the DRAM buffer pool, so isInDRAM() always returns true here and
         // cannot be used to detect a cold SSD miss.
         if(is_in_dram){
            BMC::global_bf->diag.dram_buffer_pool_hit.fetch_add(1, std::memory_order_relaxed);
         } else if(is_in_cxl){
            BMC::global_bf->diag.cxl_buffer_pool_hit.fetch_add(1, std::memory_order_relaxed);
         }
      }
      //==================================================================================================================
      // -------------------------------------------------------------------------------------
      ...
      // -------------------------------------------------------------------------------------
      // [3T-fairness] Original debug trap removed: when 3T uses BTreeVI as the cold tree,
      // bulk-loaded tuples may legitimately be reported NOT_FOUND by reconstructTuple for
      // older snapshot TXs. TwoTreeAdapter handles NOT_FOUND on the cold path via `found_cold`.
      // -------------------------------------------------------------------------------------
      ...
      jumpmuTry()
      {
         ...
         if (pos != -1) {
            ...
            if (isVisibleForMe(tuple_head.worker_id, tuple_head.tx_ts, false)) {
               ...
               //=================================================================================================================
               // [Added]. Try Lookup Interception in DRAM RecordCache
               //=================================================================================================================
               {
                  bool is_in_dram = BMC::global_bf -> isInDRAM(leaf.bf);
                  bool is_in_cxl  = !is_in_dram && FLAGS_cxl_tiering_enabled && BMC::global_bf -> isInCXL(leaf.bf);
            ...
            PARANOID_BLOCK()
            {
               ...
               // -------------------------------------------------------------------------------------
               // [3T-fairness] SIGTRAP removed: scan may legitimately reach a dead-end page
               p_guard.unlock();
            }
            jumpmu_return OP_RESULT::NOT_FOUND;
         }
                                               ...
                                               UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   jumpmuTry()
   {
      ...
      if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
         ...
         std::span<const u8> key_span(o_key, o_key_length);
         rc_invalidated =
             BMC::global_bf->global_record_cache->tryUpdateAndInvalidateRecordCache(dt_id, key_span, worker_id);
         if (rc_invalidated) {
            rc_page_id = iterator.leaf.bf->header.pid;
            rc_slot_id = static_cast<u16>(iterator.cur);
         }
      }
      if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_admission_control != nullptr) {
         const u64 page_id = iterator.leaf.bf->header.pid;
         const u16 slot_id = static_cast<u16>(iterator.cur);
         const bool is_in_dram = BMC::global_bf->isInDRAM(iterator.leaf.bf);
         const u16 page_slot_num = iterator.leaf->count;
         BMC::global_bf->getGlobalAdmissionControl().OnRecordAccess(
             page_id, slot_id, is_in_dram, page_slot_num, iterator.leaf.bf);
      }
      ...
      if (rc_invalidated && BMC::global_bf->global_record_cache != nullptr &&
          BMC::global_bf->isInCXL(iterator.leaf.bf)) {
         std::span<const u8> key_span(o_key, o_key_length);
         const u16 value_len = primary_payload.length() - sizeof(ChainedTuple);
         BMC::global_bf->global_record_cache->signalDirectUpdatePromote(
             dt_id,
             key_span,
             iterator.leaf.bf,
             rc_page_id,
      ...
      if (rc_invalidated && BMC::global_bf->global_admission_control != nullptr) {
         BMC::global_bf->getGlobalAdmissionControl().OnRecordInvalidated(rc_page_id, rc_slot_id);
      }
                                         ...
                                         UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   ...
   jumpmuTry()
   {
      ...
      if (ret != OP_RESULT::OK) {
         ...
         // -------------------------------------------------------------------------------------
         // [3T-fairness] Soft NOT_FOUND instead of SIGTRAP: 3T's TwoTreeAdapter::update1
         // probes hot, then calls updateSameSizeInPlace, but another worker's evict_a_bunch
         // may have physically removed K from hot in between. The caller retries on cold.
         jumpmu_return OP_RESULT::NOT_FOUND;
      }
      // -------------------------------------------------------------------------------------
      // Record is found
...
struct DTRegistry::DTMeta BTreeVI::getMeta()
{
   DTRegistry::DTMeta btree_meta = {.iterate_children = iterateChildrenSwips,
                                    ...
                                    // [Added]. 
                                    // Register Type callback function
                                    // If it is BTreeVI, then check if it is InnerNode, innerNode should promote to DRAM.
                                    .should_directly_promote_to_dram = [](void*, const BufferFrame& bf) -> bool {
                                       const auto& node = *reinterpret_cast<const BTreeNode*>(bf.page.dt);
                                       return node.isInner();
                                    }};
   return btree_meta;
}
