...
OP_RESULT BTreeVI::lookupPessimistic(u8* key_buffer, const u16 key_length, function<void(const u8*, u16)> payload_callback)
{
   ...
   jumpmuTry()
   {
      ...
      // -------------------------------------------------------------------------------------
      // [3T-fairness] Debug trap removed: NOT_FOUND is legitimate for cold-tree lookups.
      // -------------------------------------------------------------------------------------
                                               ...
                                               UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   jumpmuTry()
   {
      ...
      if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
         if (BMC::global_bf->global_admission_control != nullptr) {
            u64 page_id = iterator.leaf.bf->header.pid;
            BMC::global_bf->getGlobalAdmissionControl().OnRecordInvalidated(page_id, static_cast<u16>(iterator.cur));
         }
      }
      ...
      // -------------------------------------------------------------------------------------
      // Write-Through sync: page now holds new value with updated MVCC fields.
      if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
         std::span<const u8> key_span(o_key, o_key_length);
         u16 value_len = static_cast<u16>(primary_payload.length() - sizeof(ChainedTuple));
         std::span<const u8> new_value_span(tuple_head.payload, value_len);
         BMC::global_bf->global_record_cache->tryWriteThroughSync(
             key_span, new_value_span, iterator.leaf.bf,
             static_cast<u16>(iterator.cur),
             cr::Worker::my().workerID(), tuple_head.tx_ts);
      }
      tuple_head.unlock();
                                         ...
                                         UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   ...
   jumpmuTry()
   {
      ...
      if (ret != OP_RESULT::OK) {
         ...
         // -------------------------------------------------------------------------------------
         // [3T-fairness] Soft NOT_FOUND: evict_a_bunch may remove K from hot between probe and update.
         jumpmu_return OP_RESULT::NOT_FOUND;
      }
      // -------------------------------------------------------------------------------------
      // Record is found
   restart : {
      ...
      if (tuple.tuple_format == TupleFormat::FAT_TUPLE_DIFFERENT_ATTRIBUTES) {
         // FAT_TUPLE update may change the in-page value length and shifts deltas
         // around. The resident RecordCacheEntry is sized for the previous value
         // and cannot be SeqLock-synced safely; drop it and let a future Lookup
         // re-promote with the new size.
         if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
            std::span<const u8> key_span(o_key, o_key_length);
            BMC::global_bf->global_record_cache->tryInvalidateForRemove(
                key_span, cr::Worker::my().worker_id);
         }
...
OP_RESULT BTreeVI::remove(u8* o_key, u16 o_key_length)
{
   ...
   jumpmuTry()
   {
      ...
      if (FLAGS_cxl_tiering_enabled && BMC::global_bf->global_record_cache != nullptr) {
         u64 worker_id = cr::Worker::my().worker_id;
         std::span<const u8> key_span(o_key, o_key_length);
         BMC::global_bf->global_record_cache->tryInvalidateForRemove(key_span, worker_id);
         if (BMC::global_bf->global_admission_control != nullptr) {
            u64 page_id = iterator.leaf.bf->header.pid;
            BMC::global_bf->getGlobalAdmissionControl().OnRecordInvalidated(page_id, static_cast<u16>(iterator.cur));
         }
