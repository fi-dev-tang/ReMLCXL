   ...
   OP_RESULT scan(u8* o_key, u16 o_key_length, function<bool(const u8* key, u16 key_length, const u8* value, u16 value_length)> callback)
   {
      ...
      jumpmuTry()
      {
         ...
         while (ret == OP_RESULT::OK) {
            ...
            if (std::get<0>(reconstruct) == OP_RESULT::OK && FLAGS_cxl_tiering_enabled) {
               const PID cur_pid = iterator.leaf.bf->header.pid;
               if (cur_pid != last_scan_pid) {
                  last_scan_pid = cur_pid;
                  bool is_in_dram = BMC::global_bf->isInDRAM(iterator.leaf.bf);
                  bool is_in_cxl  = !is_in_dram && BMC::global_bf->isInCXL(iterator.leaf.bf);
