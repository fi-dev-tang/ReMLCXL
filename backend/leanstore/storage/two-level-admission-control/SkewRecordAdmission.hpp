...
// 16 KiB pages with small-payload tables can pack ~380 slots.
// MAX_SLOTS_PER_PAGE=192 overflowed the 3-word bitmap on TPC-C.
// std::bitset<MAX_SLOTS_PER_PAGE> implemented as std::atomic<u64>[NUM_BITMAP_WORDS]
constexpr u16 MAX_SLOTS_PER_PAGE = 512;
constexpr u16 NUM_BITMAP_WORDS   = (MAX_SLOTS_PER_PAGE + 63) / 64;
...
public:
    ...
    std::vector<PromotionDecision> CheckAndPromote(u64 l1_fine_threshold,
        ...
        const u64 round_no = check_and_promote_round_no.fetch_add(1, std::memory_order_relaxed) + 1;

        // if (local_skew_promote_count > 0 || local_uniform_promote_count > 0) {
        //     std::fprintf(stdout,
        //                  "[PROMOTE] round=%llu skew_records=%llu uniform_pages=%llu "
        //                  "decisions=%zu global_requests=%llu "
        //                  "l1_orig=%llu l1_dynamic=%llu rc_fill=%.3f\n",
        //                  static_cast<unsigned long long>(round_no),
        //                  static_cast<unsigned long long>(local_skew_promote_count),
        //                  static_cast<unsigned long long>(local_uniform_promote_count),
        //                  decisions.size(),
        //                  static_cast<unsigned long long>(global_requests.load(std::memory_order_relaxed)),
        //                  static_cast<unsigned long long>(l1_fine_threshold),
        //                  static_cast<unsigned long long>(effective_l1_fine_threshold),
        //                  record_cache_fill_ratio);
        // }
