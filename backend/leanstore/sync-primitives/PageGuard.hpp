...
class HybridPageGuard
{
  ...
  public:
   ...
   // -------------------------------------------------------------------------------------
   // I: Allocate a new page (use_split_alloc: split child page → CXL when tiering enabled; root stays DRAM)
   HybridPageGuard(DTID dt_id, bool keep_alive = true, bool use_split_alloc = false)
       : bf(use_split_alloc ? &BMC::global_bf->allocateSplitPage() : &BMC::global_bf->allocatePage()),
         guard(bf->header.latch, GUARD_STATE::EXCLUSIVE), keep_alive(keep_alive)
   {
