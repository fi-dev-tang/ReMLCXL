...
void ConfigsTable::open()
{
   ...
   //------------------------------[Added].------------------------------------------------
   columns.emplace("c_dram_partition_bits", [&](Column& col) { col << FLAGS_dram_partition_bits;});
   columns.emplace("c_cxl_partition_bits", [&](Column& col){ col << FLAGS_cxl_partition_bits;});
