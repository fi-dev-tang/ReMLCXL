...
void CRManager::joinAll()
{
   for (u32 t_i = 0; t_i < workers_count; t_i++) {
      fprintf(stdout, "[before joinAll] t=%u ready=%d job_set=%d\n",
      t_i, worker_threads_meta[t_i].wt_ready, worker_threads_meta[t_i].job_set);
      fprintf(stdout, "[joinAll] waiting worker %u\n", t_i);
      joinOne(t_i, [&](WorkerThread& meta) { 
         fprintf(stdout, "[joinAll] check worker %u: ready=%d job_set=%d\n",
         t_i, meta.wt_ready, meta.job_set);
         return meta.wt_ready && !meta.job_set; });
         fprintf(stdout, "[joinAll] worker %u done\n", t_i);
   }
}
// -------------------------------------------------------------------------------------
...
void CRManager::joinOne(u64 t_i, std::function<bool(WorkerThread&)> condition)
{
   ...
   fprintf(stdout, "[joinOne] t=%lu enter: ready=%d job_set=%d\n", t_i, meta.wt_ready, meta.job_set);

   meta.cv.wait(guard, [&]() { 
      fprintf(stdout, "[joinOne] t=%lu check: ready=%d job_set=%d\n",
      t_i, meta.wt_ready, meta.job_set);
      return condition(meta); 
   });

   fprintf(stdout, "[joinOne] t=%lu exit: ready=%d job_set=%d\n",
      t_i, meta.wt_ready, meta.job_set);
} 
// -------------------------------------------------------------------------------------
