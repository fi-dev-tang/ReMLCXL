  ...
  if (activeTX().isDurable()) {
    {
      ...
      // Use JMUW instead of plain std::unique_lock so that jumpmu::jump()
      // (longjmp) will properly unlock the mutex via the destructor stack.
      // A plain std::unique_lock would leak the lock if longjmp fires while
      // the mutex is held, causing a deadlock in the Group Committer.
      JMUW<std::unique_lock<std::mutex>> g(logging.precommitted_queue_mutex);
