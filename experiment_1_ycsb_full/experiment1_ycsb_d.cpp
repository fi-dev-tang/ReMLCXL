...
// ycsb_d_test.cpp
//
// YCSB-D (95% read-latest, 5% insert) workload simulation for CXL record
// cache experiments.
//
// YCSB-D "read latest" semantics:
//   - 5% of operations insert a new record with a monotonically increasing key
//   - 95% of operations read an existing record, biased toward the most
//     recently inserted keys via a Zipfian distribution over the *current*
//     key space (which grows as inserts happen)
//   - The Zipfian skew is applied such that rank 0 maps to the newest key,
//     i.e. key = current_max - zipf_rank, favouring "latest" records
//
// Key-generation for reads follows ScrambledZipfGenerator but the item
// count is the *current* (growing) number of records rather than a fixed
// value.  Insert keys are allocated from an atomic counter starting at
                      ...
                      u64 payload_size_bytes)
{
   ...
   std::atomic<u64> loaded{0};
   const u64 progress_interval = 100000;
       ...
       [&](u64 t_i, u64 range_begin, u64 range_end) {
          crm.scheduleJobAsync(t_i, [&, range_begin, range_end]() {
             for (u64 i = range_begin; i < range_end; i++) {
                ...
                u64 done = loaded.fetch_add(1, std::memory_order_relaxed) + 1;
                if (done % progress_interval == 0) {
                   double elapsed = std::chrono::duration<double>(
                       std::chrono::high_resolution_clock::now() - t0).count();
                   double pct = 100.0 * done / tuple_count;
                   int bar_width = 30;
                   int filled = static_cast<int>(pct / 100.0 * bar_width);
                   std::string bar(bar_width, '-');
                   std::fill(bar.begin(), bar.begin() + filled, '#');
                   std::cout << "[Load] [" << bar << "] " << done << "/" << tuple_count
                             << " (" << pct << "%)"
                             << " elapsed=" << elapsed << "s"
                             << " rate=" << (done / elapsed / 1e6) << " M rec/s"
                             << std::endl;
                }
             }
          });
...
// ============================================================================
// Run phase — YCSB-D: 95% read-latest + 5% insert with growing key space
// ============================================================================
                                ...
                                bool measured)
{
   ...
   for (u64 t_i = 0; t_i < thread_count; t_i++) {
      ...
      crm.scheduleJobAsync(t_i, [&, t_i, quota]() {
         ...
         u64 cached_max = next_insert_key.load(std::memory_order_relaxed);
         ScrambledZipfGenerator latest_gen(cached_max, cfg.zipf_theta);
            ...
            jumpmuTry() {
               ...
               if (op < 0.95) {
                  ...
                  if (current_max != cached_max) {
                     latest_gen.adjustN(current_max);
                     cached_max = current_max;
                  }
                  const u64 zipf_rank = latest_gen.next(rng);
                  const YCSBKey key = (current_max > zipf_rank) ? (current_max - 1 - zipf_rank) : 0;
                  ...
                  table.insert_var({new_key}, {payload}, cfg.payload_size_bytes);
                  found = true;
               }
               cr::Worker::my().commitTX();
...
int main(int argc, char** argv)
{
   gflags::SetUsageMessage(
       "YCSB-D (95% read, 5% insert) simulation for CXL record cache experiments");
   ...
   std::cout << "\n" << Color::BOLD << Color::CYAN
             << "============================================\n"
             << "  YCSB-D Test (95% Read, 5% Insert, ScrambledZipf)\n"
             << "============================================"
             << Color::RESET << "\n";
   ...
   // not loaded during bulk-load).  Grows as YCSB-D inserts new records.
   ...
   crm.scheduleJobSync(0, [&]() {
      table = LeanStoreAdapter<YCSBTable>(db, "YCSB_D_TEST");
   });
   ...
   // YCSB-D: skip ConfigureDynamicMaxPerPageVisits — "read latest" distribution
   // causes excessive full-page promotions with aggressive dynamic thresholds.
   // Falls back to FLAGS_max_per_page_visits (default 8000).

   // Phase 2: warmup
   double warmup_secs = 0.0;
   if (FLAGS_test_warmup_lookups > 0) {
      print_phase("Phase 2: Warmup Lookup");
      auto warmup_stats = run_lookup_phase(crm, table, cfg, next_insert_key,
   ...
   // Phase 3: measured YCSB-D run
   print_phase("Phase 3: Measured (YCSB-D, 95% Read-Latest, 5% Insert)");
   const LookupRunStats stats = run_lookup_phase(
       crm, table, cfg, next_insert_key,
   ...
   print_pass("YCSB-D test finished.");
   return 0;
}