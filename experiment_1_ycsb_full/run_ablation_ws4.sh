...
# =============================================================================
# Ablation experiment: working_set = 4 GiB
#
...
#
# Warmup policy:
#   Tier 1 (A/B/C/F) warmup = 100M  (scaled with ws — this script: ws=4 GiB)
...
# When run standalone, fall back to a local experiment_formal_ws4_<ts> dir.
if [[ -n "${EXPERIMENT_ROOT:-}" ]]; then
   RESULT_DIR="${EXPERIMENT_ROOT}/ws4"
else
   RESULT_DIR="$SCRIPT_DIR/experiment_formal_ws4_${TIMESTAMP}"
fi
mkdir -p "$RESULT_DIR"
...
# -----------------------------------------------------------------------------
# Working-set / CXL / warmup config (this script: ws = 4 GiB)
# -----------------------------------------------------------------------------
WORKING_SET_GIB=4.0
PAYLOAD_SIZE_BYTES=100
CXL_GIB=2.5

WARMUP_LOOKUPS_TIER1=100000000          # A/B/C/F  (1:1 with measure; longer warmup pushes L1 threshold into a dead zone)
WARMUP_LOOKUPS_TIER2=30000000           # D/E
MEASURE_LOOKUPS=100000000               # 25M ops/GiB x 4 GiB
