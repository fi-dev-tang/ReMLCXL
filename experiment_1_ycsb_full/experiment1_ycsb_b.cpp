...
// ycsb_b_test.cpp
//
// YCSB-B (95% read, 5% update) workload simulation for CXL record cache experiments.
//
...
struct CorrectedCounters {
   ...
   // Urgent fast-promote funnel
   u64 urgent_not_marked_candidate_missing = 0;
   u64 urgent_marked_total                 = 0;
   u64 urgent_seen_in_check                = 0;
   u64 urgent_seen_but_already_promoted    = 0;
   u64 urgent_decision_generated           = 0;
   u64 urgent_enqueued_to_promote          = 0;
   u64 normal_enqueued_to_promote          = 0;
   u64 urgent_fail_cxl_bf_stale            = 0;
   u64 urgent_fail_slot_boundary           = 0;
   u64 urgent_fail_large_record            = 0;
   u64 normal_fail_cxl_bf_stale            = 0;
   u64 normal_fail_slot_boundary           = 0;
   u64 normal_fail_large_record            = 0;

   // Promote thread stage diagnostics
   u64 promote_rejected_high_water = 0;
   u64 promote_alloc_failed        = 0;
   u64 promote_head_still_live     = 0;
   u64 promote_cxl_read_failed     = 0;
   u64 promote_success             = 0;
   u64 promote_enqueue_skipped_inflight = 0;
   u64 promote_enqueue_skipped_queue_full = 0;
   u64 direct_update_enqueued = 0;
   u64 direct_update_skipped_inflight = 0;
   u64 direct_update_skipped_queue_full = 0;
   u64 direct_update_promote_success = 0;
   u64 direct_update_head_still_live = 0;
   u64 update_cancelled_placeholder = 0;
   u64 promote_cancelled_before_publish = 0;
   // Update-path diagnostics for placeholder-skipping strategy
   u64 update_skip_placeholder     = 0;
   u64 update_found_readonly_next  = 0;
   u64 update_no_updatable_readonly= 0;
...
static PromoteEfficiencyMetrics compute_promote_efficiency_metrics(const CorrectedCounters& c)
{
   ...
   const u64 total_req = c.urgent_enqueued_to_promote + c.normal_enqueued_to_promote;
   if (total_req == 0) {
      return m;
   }
   const double total_req_d = static_cast<double>(total_req);
   const double success_new_insert = static_cast<double>(c.promote_success);
   const double wasted = static_cast<double>(c.promote_rejected_high_water) +
                         static_cast<double>(c.promote_alloc_failed) +
                         static_cast<double>(c.promote_head_still_live) +
                         static_cast<double>(c.promote_cxl_read_failed);
   m.useful_promote = success_new_insert / total_req_d;
   m.wasted_promote = wasted / total_req_d;
   return m;
}
...
static CorrectedCounters read_and_correct_counters()
{
   ...
   const u64 urgent_not_marked_candidate_missing =
       bm.diag.urgent_not_marked_candidate_missing.load(std::memory_order_relaxed);
   const u64 urgent_marked_total =
       bm.diag.urgent_marked_total.load(std::memory_order_relaxed);
   // These 3 counters live in DramHotPageCandidates (not diag) because
   // CheckAndPromote in SkewRecordAdmission.hpp cannot see BMC.
   u64 urgent_seen_in_check = 0;
   u64 urgent_seen_but_already_promoted = 0;
   u64 urgent_decision_generated = 0;
   if (auto* wrapper = bm.getAdmissionControlWrapper(); wrapper != nullptr) {
      auto& hc = wrapper->GetHotPageCandidates();
      urgent_seen_in_check = hc.GetUrgentSeenInCheck();
      urgent_seen_but_already_promoted = hc.GetUrgentSeenButAlreadyPromoted();
      urgent_decision_generated = hc.GetUrgentDecisionGenerated();
   }
   const u64 urgent_enqueued_to_promote =
       bm.diag.urgent_enqueued_to_promote.load(std::memory_order_relaxed);
   const u64 normal_enqueued_to_promote =
       bm.diag.normal_enqueued_to_promote.load(std::memory_order_relaxed);
   const u64 urgent_fail_cxl_bf_stale =
       bm.diag.urgent_fail_cxl_bf_stale.load(std::memory_order_relaxed);
   const u64 urgent_fail_slot_boundary =
       bm.diag.urgent_fail_slot_boundary.load(std::memory_order_relaxed);
   const u64 urgent_fail_large_record =
       bm.diag.urgent_fail_large_record.load(std::memory_order_relaxed);
   const u64 normal_fail_cxl_bf_stale =
       bm.diag.normal_fail_cxl_bf_stale.load(std::memory_order_relaxed);
   const u64 normal_fail_slot_boundary =
       bm.diag.normal_fail_slot_boundary.load(std::memory_order_relaxed);
   const u64 normal_fail_large_record =
       bm.diag.normal_fail_large_record.load(std::memory_order_relaxed);
   const u64 record_promote_requests =
       urgent_enqueued_to_promote + normal_enqueued_to_promote;

   const u64 promote_rejected_high_water =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteRejectedHighWater()
           : 0;
   const u64 promote_alloc_failed =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteAllocFailed()
           : 0;
   const u64 promote_head_still_live =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteHeadStillLive()
           : 0;
   const u64 promote_cxl_read_failed =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteCxlReadFailed()
           : 0;
   const u64 promote_success =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteSuccess()
           : 0;
   const u64 promote_enqueue_skipped_inflight =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteEnqueueSkippedInflight()
           : 0;
   const u64 promote_enqueue_skipped_queue_full =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteEnqueueSkippedQueueFull()
           : 0;
   const u64 direct_update_enqueued =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetDirectUpdateEnqueued()
           : 0;
   const u64 direct_update_skipped_inflight =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetDirectUpdateSkippedInflight()
           : 0;
   const u64 direct_update_skipped_queue_full =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetDirectUpdateSkippedQueueFull()
           : 0;
   const u64 direct_update_promote_success =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetDirectUpdatePromoteSuccess()
           : 0;
   const u64 direct_update_head_still_live =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetDirectUpdateHeadStillLive()
           : 0;
   const u64 update_cancelled_placeholder =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetUpdateCancelledPlaceholder()
           : 0;
   const u64 promote_cancelled_before_publish =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetPromoteCancelledBeforePublish()
           : 0;
   const u64 update_skip_placeholder =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetUpdateSkipPlaceholder()
           : 0;
   const u64 update_found_readonly_next =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetUpdateFoundReadonlyNext()
           : 0;
   const u64 update_no_updatable_readonly =
       (bm.global_record_cache != nullptr)
           ? bm.global_record_cache->GetUpdateNoUpdatableReadonly()
           : 0;
   ...
   c.urgent_not_marked_candidate_missing = urgent_not_marked_candidate_missing;
   c.urgent_marked_total = urgent_marked_total;
   c.urgent_seen_in_check = urgent_seen_in_check;
   c.urgent_seen_but_already_promoted = urgent_seen_but_already_promoted;
   c.urgent_decision_generated = urgent_decision_generated;
   c.urgent_enqueued_to_promote = urgent_enqueued_to_promote;
   c.normal_enqueued_to_promote = normal_enqueued_to_promote;
   c.urgent_fail_cxl_bf_stale = urgent_fail_cxl_bf_stale;
   c.urgent_fail_slot_boundary = urgent_fail_slot_boundary;
   c.urgent_fail_large_record = urgent_fail_large_record;
   c.normal_fail_cxl_bf_stale = normal_fail_cxl_bf_stale;
   c.normal_fail_slot_boundary = normal_fail_slot_boundary;
   c.normal_fail_large_record = normal_fail_large_record;
   c.promote_rejected_high_water = promote_rejected_high_water;
   c.promote_alloc_failed = promote_alloc_failed;
   c.promote_head_still_live = promote_head_still_live;
   c.promote_cxl_read_failed = promote_cxl_read_failed;
   c.promote_success = promote_success;
   c.promote_enqueue_skipped_inflight = promote_enqueue_skipped_inflight;
   c.promote_enqueue_skipped_queue_full = promote_enqueue_skipped_queue_full;
   c.direct_update_enqueued = direct_update_enqueued;
   c.direct_update_skipped_inflight = direct_update_skipped_inflight;
   c.direct_update_skipped_queue_full = direct_update_skipped_queue_full;
   c.direct_update_promote_success = direct_update_promote_success;
   c.direct_update_head_still_live = direct_update_head_still_live;
    c.update_cancelled_placeholder = update_cancelled_placeholder;
    c.promote_cancelled_before_publish = promote_cancelled_before_publish;
   c.update_skip_placeholder = update_skip_placeholder;
   c.update_found_readonly_next = update_found_readonly_next;
   c.update_no_updatable_readonly = update_no_updatable_readonly;
...
struct LookupRunStats {
   ...
   u64    rc_lookup_total   = 0;
   double rc_lookup_hr      = 0.0;
...
static void reset_diag_counters()
{
   ...
   bm.diag.urgent_not_marked_candidate_missing.store(0, std::memory_order_relaxed);
   bm.diag.urgent_marked_total.store(0, std::memory_order_relaxed);
   // Reset DramHotPageCandidates urgent counters (moved from diag)
   if (auto* wrapper = bm.getAdmissionControlWrapper(); wrapper != nullptr) {
      wrapper->GetHotPageCandidates().ResetUrgentDiagCounters();
   }
   bm.diag.urgent_enqueued_to_promote.store(0, std::memory_order_relaxed);
   bm.diag.normal_enqueued_to_promote.store(0, std::memory_order_relaxed);
   bm.diag.urgent_fail_cxl_bf_stale.store(0, std::memory_order_relaxed);
   bm.diag.urgent_fail_slot_boundary.store(0, std::memory_order_relaxed);
   bm.diag.urgent_fail_large_record.store(0, std::memory_order_relaxed);
   bm.diag.normal_fail_cxl_bf_stale.store(0, std::memory_order_relaxed);
   bm.diag.normal_fail_slot_boundary.store(0, std::memory_order_relaxed);
   bm.diag.normal_fail_large_record.store(0, std::memory_order_relaxed);
   if (bm.global_record_cache != nullptr) {
      bm.global_record_cache->ResetPromoteDiagCounters();
   }
}

// ============================================================================
// Lookup phase — YCSB-B: 95% read, 5% update with ScrambledZipf key distribution
// ============================================================================
                                ...
                                bool measured)
{
   ...
   std::atomic<u64> read_ops{0};
   std::atomic<u64> write_ops{0};
         ...
         while (local_done < quota) {
            ...
            jumpmuTry() {
               ...
               if (is_read_op) {
                  ...
                  update_desc.slots[0].length = cfg.payload_size_bytes;
                  YCSBPayload payload;
                  std::memset(payload.value, 0, sizeof(payload.value));
                  utils::RandomGenerator::getRandString(payload.value, cfg.payload_size_bytes);
                  table.update1({key}, [&](YCSBTable& rec) {
                     std::memcpy(rec.my_payload.value, payload.value, cfg.payload_size_bytes);
                  }, update_desc);
                  found = true;
               }
               cr::Worker::my().commitTX();
   ...
   if (measured) {
      ...
      stats.rc_lookup_total = c.rc_hit + c.rc_miss;
      stats.rc_lookup_hr =
          (stats.rc_lookup_total > 0)
              ? (100.0 * static_cast<double>(c.rc_hit) /
                 static_cast<double>(stats.rc_lookup_total))
              : 0.0;
      ...
      print_info("[UrgentFunnel][entry] marked=" + std::to_string(c.urgent_marked_total) +
                 ", candidate_missing=" + std::to_string(c.urgent_not_marked_candidate_missing));
      print_info("[UrgentFunnel][decision] seen=" + std::to_string(c.urgent_seen_in_check) +
                 ", seen_but_promoted=" + std::to_string(c.urgent_seen_but_already_promoted) +
                 ", generated=" + std::to_string(c.urgent_decision_generated));
      print_info("[UrgentFunnel][enqueue] urgent=" + std::to_string(c.urgent_enqueued_to_promote) +
                 ", normal=" + std::to_string(c.normal_enqueued_to_promote));
      print_info("[UrgentFunnel][pre-enqueue-fail] urgent(stale/slot/large)=" +
                 std::to_string(c.urgent_fail_cxl_bf_stale) + "/" +
                 std::to_string(c.urgent_fail_slot_boundary) + "/" +
                 std::to_string(c.urgent_fail_large_record) +
                 ", normal(stale/slot/large)=" +
                 std::to_string(c.normal_fail_cxl_bf_stale) + "/" +
                 std::to_string(c.normal_fail_slot_boundary) + "/" +
                 std::to_string(c.normal_fail_large_record));
      print_info("[UrgentFunnel][promote-thread] success=" + std::to_string(c.promote_success) +
                 ", high_water_reject=" + std::to_string(c.promote_rejected_high_water) +
                 ", alloc_fail=" + std::to_string(c.promote_alloc_failed) +
                 ", head_still_live=" + std::to_string(c.promote_head_still_live) +
                 ", cxl_read_fail=" + std::to_string(c.promote_cxl_read_failed));
      print_info("[PromoteCancel] update_cancelled_placeholder=" +
                 std::to_string(c.update_cancelled_placeholder) +
                 ", promote_cancelled_before_publish=" +
                 std::to_string(c.promote_cancelled_before_publish));
      print_info("[PromoteQueue] skipped_inflight=" +
                 std::to_string(c.promote_enqueue_skipped_inflight) +
                 ", skipped_queue_full=" +
                 std::to_string(c.promote_enqueue_skipped_queue_full) +
                 ", direct_update_enqueued=" +
                 std::to_string(c.direct_update_enqueued) +
                 ", direct_update_skipped_inflight=" +
                 std::to_string(c.direct_update_skipped_inflight) +
                 ", direct_update_skipped_queue_full=" +
                 std::to_string(c.direct_update_skipped_queue_full) +
                 ", direct_update_promote_success=" +
                 std::to_string(c.direct_update_promote_success) +
                 ", direct_update_head_still_live=" +
                 std::to_string(c.direct_update_head_still_live));
      print_info("[UpdatePath][readonly-only] skip_placeholder=" +
                 std::to_string(c.update_skip_placeholder) +
                 ", found_readonly_next=" + std::to_string(c.update_found_readonly_next) +
                 ", no_updatable_readonly=" + std::to_string(c.update_no_updatable_readonly));
      print_info("--- Isolated Lookup-Only Metrics (NOT mixed with total-path counters) ---");
      print_info("ops(read/update)=" + std::to_string(stats.read_ops) + "/" +
                 std::to_string(stats.write_ops));
      print_info("RC_lookup_only: hit=" + std::to_string(c.rc_hit) +
                 ", miss=" + std::to_string(c.rc_miss) +
                 ", total_lookup_path=" + std::to_string(stats.rc_lookup_total) +
                 ", RC_lookup_HR=" + std::to_string(stats.rc_lookup_hr) + "%");
...
int main(int argc, char** argv)
{
   gflags::SetUsageMessage(
       "YCSB-B (95% read, 5% update) simulation for CXL record cache experiments");
   ...
   std::cout << "\n" << Color::BOLD << Color::CYAN
             << "============================================\n"
             << "  YCSB-B Test (95% Read, 5% Update, ScrambledZipf)\n"
             << "============================================"
             << Color::RESET << "\n";
   ...
   crm.scheduleJobSync(0, [&]() {
      table = LeanStoreAdapter<YCSBTable>(db, "YCSB_B_TEST");
   });
   ...
   print_phase("Phase 3: Measured Lookup (YCSB-B, 95% Read, 5% Update)");
   ...
   // ============================================================================
   {
      ...
      print_info("  Urgent funnel(entry/decision/enqueue) = marked:" +
                 std::to_string(c.urgent_marked_total) +
                 " missing:" + std::to_string(c.urgent_not_marked_candidate_missing) +
                 " seen:" + std::to_string(c.urgent_seen_in_check) +
                 " generated:" + std::to_string(c.urgent_decision_generated) +
                 " urgent_enqueue:" + std::to_string(c.urgent_enqueued_to_promote) +
                 " normal_enqueue:" + std::to_string(c.normal_enqueued_to_promote));
      print_info("  Urgent pre-enqueue fail(stale/slot/large) = " +
                 std::to_string(c.urgent_fail_cxl_bf_stale) + "/" +
                 std::to_string(c.urgent_fail_slot_boundary) + "/" +
                 std::to_string(c.urgent_fail_large_record));
      print_info("  Normal pre-enqueue fail(stale/slot/large) = " +
                 std::to_string(c.normal_fail_cxl_bf_stale) + "/" +
                 std::to_string(c.normal_fail_slot_boundary) + "/" +
                 std::to_string(c.normal_fail_large_record));
      print_info("  Promote-thread(success/high_water/alloc/head_live/cxl_read_fail) = " +
                 std::to_string(c.promote_success) + "/" +
                 std::to_string(c.promote_rejected_high_water) + "/" +
                 std::to_string(c.promote_alloc_failed) + "/" +
                 std::to_string(c.promote_head_still_live) + "/" +
                 std::to_string(c.promote_cxl_read_failed));
      print_info("  Promote cancel(update_cancelled/promote_cancelled_before_publish) = " +
                 std::to_string(c.update_cancelled_placeholder) + "/" +
                 std::to_string(c.promote_cancelled_before_publish));
      print_info("  Promote queue(skipped_inflight/skipped_queue_full/direct_enq/direct_skip_inflight/direct_skip_queue_full/direct_success/direct_head_live) = " +
                 std::to_string(c.promote_enqueue_skipped_inflight) + "/" +
                 std::to_string(c.promote_enqueue_skipped_queue_full) + "/" +
                 std::to_string(c.direct_update_enqueued) + "/" +
                 std::to_string(c.direct_update_skipped_inflight) + "/" +
                 ...
                 std::to_string(c.direct_update_promote_success) + "/" +
                 std::to_string(c.direct_update_head_still_live));
      print_info("  Update-path(readonly-only) skip_placeholder/found_next/no_updatable = " +
                 std::to_string(c.update_skip_placeholder) + "/" +
                 std::to_string(c.update_found_readonly_next) + "/" +
                 std::to_string(c.update_no_updatable_readonly));
      print_info("--- Isolated Lookup-Only Metrics (NOT mixed with above total-path breakdown) ---");
      print_info("  ops(read/update) = " + std::to_string(stats.read_ops) +
                 "/" + std::to_string(stats.write_ops));
      print_info("  RC_lookup_HR     = " + std::to_string(stats.rc_lookup_hr) + "%" +
                 "  [hit=" + std::to_string(c.rc_hit) +
                 ", miss=" + std::to_string(c.rc_miss) +
                 ", total=" + std::to_string(stats.rc_lookup_total) + "]");
   ...
   print_pass("YCSB-B test finished.");
   return 0;
}