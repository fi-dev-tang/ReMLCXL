...
# Quick smoke test: two_level (full-system) across YCSB A/B/C/D/E/F
# Based on optimization_step2_large_in_memory_ycsb_bc.sh config
# Reduced warmup/measure for fast verification

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
BUILD_DIR="${BUILD_DIR:-$REPO_ROOT/build/frontend}"
RESULT_DIR="${RESULT_DIR:-$SCRIPT_DIR/result_quick_test_two_level}"

# -------- Workload scope --------
WORKLOADS=(a b c d e f)
THETAS=(${THETAS:-0.90})

# -------- Scale knobs (small for quick test) --------
WORKING_SET_GIB="${WORKING_SET_GIB:-2.0}"
PAYLOAD_SIZE_BYTES="${PAYLOAD_SIZE_BYTES:-100}"
WARMUP_LOOKUPS="${WARMUP_LOOKUPS:-5000000}"
MEASURE_LOOKUPS="${MEASURE_LOOKUPS:-10000000}"
...
TOTAL=$(( ${#WORKLOADS[@]} * ${#THETAS[@]} ))
CURRENT=0

log "Quick test: two_level (full-system) YCSB A-F"
log "workloads=${WORKLOADS[*]} thetas=${THETAS[*]} ws=${WORKING_SET_GIB}GiB warmup=${WARMUP_LOOKUPS} measure=${MEASURE_LOOKUPS}"
log "dram_bp=${DRAM_BP_TWO_LEVEL} dram_rc=${DRAM_RC_TWO_LEVEL} cxl=${CXL_GIB}"
...
for theta in "${THETAS[@]}"; do
  for wl in "${WORKLOADS[@]}"; do
    CURRENT=$((CURRENT + 1))
    bin="$(get_binary "$wl")"
    result_file="$RESULT_DIR/result_ycsb${wl}_two_level_theta${theta}_quick_${TIMESTAMP}.csv"
    ...
    log "[${CURRENT}/${TOTAL}] RUN wl=${wl} theta=${theta} -> $result_file"
    ...
    if [[ "$exit_code" -ne 0 ]]; then
      log "WARN: wl=$wl theta=$theta exit_code=$exit_code"
    else
      log "OK: wl=$wl theta=$theta completed"
    fi
