...
// ycsb_f_test.cpp
//
// YCSB-F (50% read, 50% read-modify-write) workload simulation for CXL record cache experiments.
//
...
// ============================================================================
// Lookup phase — YCSB-F: 50% read, 50% read-modify-write with ScrambledZipf key distribution
// ============================================================================
                                ...
                                bool measured)
{
   ...
   if (measured) {
      ...
      print_info("ops(read/rmw)=" + std::to_string(stats.read_ops) + "/" +
...
int main(int argc, char** argv)
{
   gflags::SetUsageMessage(
       "YCSB-F (50% read, 50% rmw) simulation for CXL record cache experiments");
   ...
   std::cout << "\n" << Color::BOLD << Color::CYAN
             << "============================================\n"
             << "  YCSB-F Test (50% Read, 50% RMW, ScrambledZipf)\n"
             << "============================================"
             << Color::RESET << "\n";
   ...
   crm.scheduleJobSync(0, [&]() {
      table = LeanStoreAdapter<YCSBTable>(db, "YCSB_F_TEST");
   });
   ...
   if (FLAGS_test_warmup_lookups > 0) {
      ...
      auto warmup_stats = run_lookup_phase(crm, table, cfg, generator,
   ...
   print_phase("Phase 3: Measured Lookup (YCSB-F, 50% Read, 50% RMW)");
   const LookupRunStats stats = run_lookup_phase(
       crm, table, cfg, generator,
   ...
   print_pass("YCSB-F test finished.");
   return 0;
}