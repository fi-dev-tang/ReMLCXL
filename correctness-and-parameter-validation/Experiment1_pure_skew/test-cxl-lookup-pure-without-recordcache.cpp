...
//
// Goals:
//   1) Ensure lookup key space matches loaded key space
//   2) Measure DRAM / CXL hit statistics in "without RecordCache" ablation
//   3) Provide a workload sanity report
//
...
// Run:   ./build/frontend/cxl_lookup_pure_without_recordcache_test \
...
//   --pp_threads=1 \
//   --cxl_pp_threads=1 \
//   --ssd_path=/path/to/project/cxl-recordcache-dev/tmp/cxl_test_ssd \
//   --trunc=true \
//   --wal=true \
//   --test_working_set_gib=4 \
//   --test_zipf_theta=0.99 \
//   --test_warmup_lookups=2000000 \
//   --test_measure_lookups=300000000 \
//   --delay_admission_recordcache_threads_start=true \
//   --enable_record_cache=false \
//   2>&1 | tee "result_without_rc_$(date +%Y%m%d_%H%M%S).log"
// =============================================================================
...
//   - warmup phase
//   - measured phase
//   - collect DRAM/CXL stats
//   - collect found/not-found stats
// ============================================================================
void run_lookup_phase(cr::CRManager& crm,
      ...
      crm.scheduleJobAsync(t_i, [&, t_i, quota]() {
         ...
         while (local_done < quota) {
            jumpmuTry()
            {
               ...
               constexpr u64 kProgressEvery = 1000000ULL;
               if (measured && (g % kProgressEvery == 0)) {
                  ...
                  const double dram_hr = (tier_hits > 0) ? (100.0 * dram_h / tier_hits) : 0.0;
                  const double cxl_hr = (tier_hits > 0) ? (100.0 * cxl_h / tier_hits) : 0.0;

                  print_info("Progress " + std::to_string(g) + "/" + std::to_string(lookup_count) +
                             ", DRAM hit=" + std::to_string(dram_h) +
                             " (" + std::to_string(dram_hr) + "%)" +
                             ", CXL hit=" + std::to_string(cxl_h) +
                             " (" + std::to_string(cxl_hr) + "%)");
               }
            }
            jumpmuCatch()
            {
   ...
   if (measured) {
      ...
      print_info("DRAM BufferPool hit=" + std::to_string(dram_h) +
                 ", hit_ratio_within_tiers=" + std::to_string(dram_hr) + "%");
      print_info("CXL  BufferPool hit=" + std::to_string(cxl_h) +
                 ", hit_ratio_within_tiers=" + std::to_string(cxl_hr) + "%");
...
int main(int argc, char** argv) {
   ...
   // This ablation test disables RecordCache by default.
   ...
   if (FLAGS_test_warmup_lookups > 0) {
      ...
      run_lookup_phase(crm, table, cfg, generator, FLAGS_test_warmup_lookups, false);
   ...
   run_lookup_phase(crm, table, cfg, generator, FLAGS_test_measure_lookups, true);
