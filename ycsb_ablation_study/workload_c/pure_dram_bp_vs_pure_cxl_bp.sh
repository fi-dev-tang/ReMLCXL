...
# --- Sweep dimensions --------------------------------------------------------
WORKING_SET_GIB_LIST=(16 32 64 128 256)
ZIPF_THETA_LIST=(0.80 0.85 0.90 0.95 0.99)
DRAM_PCT_LIST=(25 50 75 85 90)   # DRAM baseline = working_set × pct%

# --- Fixed parameters --------------------------------------------------------
PAYLOAD_SIZE_BYTES=100
CXL_GIB=2.0

# DRAM_BASELINE 按 working set 动态计算，见 compute_dram_baseline()
...
RESULT_DIR="$SCRIPT_DIR/pure_dram_vs_pure_cxl/"
...
# 根据 working set 大小和百分比动态算 DRAM baseline
# DRAM_BASELINE = working_set_gib × (pct / 100)
compute_dram_baseline() {
    local ws_gib="$1"
    local pct="$2"
    awk "BEGIN { printf \"%.3f\", $ws_gib * $pct / 100.0 }"
}

# 根据 working set 大小决定预分配 SSD 空间
# 留出足够余量：working_set * 1.5，向上取整到 GiB
compute_falloc_gib() {
    local ws_gib="$1"
...
run_mode() {
    local label="$1"
    local mode="$2"
    # 文件名中嵌入 ws、theta 和 dram 大小，方便后续批量解析
    local result_file
    result_file="$RESULT_DIR/fast_result_ycsbc_${label}_ws${CURRENT_WS_GIB}gib_dram${CURRENT_DRAM_BASELINE}gib_theta${CURRENT_THETA}_${TIMESTAMP}.csv"
    shift 2
    local extra_flags=("$@")
...
run_one_combination() {
    ...
    # 遍历每个 DRAM 百分比
    for dram_pct in "${DRAM_PCT_LIST[@]}"; do
        local dram_baseline
        dram_baseline=$(compute_dram_baseline "$ws_gib" "$dram_pct")
        ...
        log_phase "=== Combination: ws=${ws_gib} GiB | theta=${theta} | dram_pct=${dram_pct}% | dram=${dram_baseline} GiB ==="

        # ------------------------------------------------------------------
        # Run A: pure DRAM + SSD
        # ------------------------------------------------------------------
        log_phase "Run A: pure DRAM + SSD  (ws=${ws_gib}, theta=${theta}, dram_pct=${dram_pct}%)"

        NUMA_MEMBIND_NODE="" \
        run_mode "dram_ssd_pct${dram_pct}" "page_only" \
        ...
        # Run B: pure CXL + SSD
        # ------------------------------------------------------------------
        log_phase "Run B: pure CXL + SSD  (ws=${ws_gib}, theta=${theta}, dram_pct=${dram_pct}%)"
        ...
        run_mode "pure_cxl_pct${dram_pct}" "page_only" \
            --cxl_tiering_enabled=false \
            --dram_buffer_pool_gib="$dram_baseline" \
            --pp_threads="$PP_THREADS"
...
total_dram_pcts=${#DRAM_PCT_LIST[@]}
total_runs=$(( total_combinations * total_dram_pcts * 2 ))   # dram + cxl, per pct
...
echo "  dram_pct_list        = ${DRAM_PCT_LIST[*]}%"
...
echo "  dram_pct variants    = $total_dram_pcts"
echo "  total runs           = $total_runs  (× 2 modes × ${total_dram_pcts} pcts)"
