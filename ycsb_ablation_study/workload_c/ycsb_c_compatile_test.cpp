...
DEFINE_string(test_distribution,           "zipf",         "Distribution: zipf | uniform");
      ...
      crm.scheduleJobAsync(t_i, [&, t_i, quota]() {
         ...
         while (local_done < quota) {
            ...
            jumpmuTry() {
               ...
               const YCSBKey key = (FLAGS_test_distribution == "uniform")
                                   ? (rng.next() % cfg.total_records())
                                   : generator.next(rng);
               bool          found = false;
               ...
               if (FLAGS_test_progress_interval > 0 &&
                   global_done % FLAGS_test_progress_interval == 0) {
...
int main(int argc, char** argv)
{
   ...
   if (FLAGS_test_warmup_lookups > 0) {
      ...
      (void)run_lookup_phase(crm, table, cfg, generator,
